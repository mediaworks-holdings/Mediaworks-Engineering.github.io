...
function canvasBufferWindowUpdate(canvas, minTime, maxTime, focusTime, events) {
    ...
    if (firstEventIdx + 1 < events.length) {
        ...
        minTimeBuffer =
            minTimePos + events[firstEventIdx].buffer + ((minTime - events[firstEventIdx].time) * (events[firstEventIdx + 1].buffer - events[firstEventIdx].buffer)) / (events[firstEventIdx + 1].time - events[firstEventIdx].time);
    } else {
        minTimeBuffer = 0;
        minTimePos = 0;
    }
...
function canvasBitrateEventUpdate(canvas, minTime, maxTime, windowMinTime, windowMaxTime, levelEvents, bitrateEvents) {
    ...
    ctx.font = "15px Arial";
    y_offset += 15;
    var legend = "last bitrate:" + (bitrateEvents[bitrateEvents.length - 1].bitrate / 1e3).toFixed(2) + "Mb/s";
    ctx.fillText(legend, x_offset, y_offset);
    y_offset += 15;
    legend = "min bitrate:" + (minBitrate / 1e3).toFixed(2) + "Mb/s";
    ctx.fillText(legend, x_offset, y_offset);
    y_offset += 15;
    legend = "max bitrate:" + (maxBitrate / 1e3).toFixed(2) + "Mb/s";
    ctx.fillText(legend, x_offset, y_offset);
    y_offset += 15;
...
function canvasDrawLoadEvent(ctx, yoffset, event, minTime, maxTime) {
    ...
    if (event.size) {
        if (event.size > 1e3 * 1e3) {
            legend = (event.size / 1e6).toFixed(1) + "MB";
        } else {
            legend = Math.round(event.size / 1e3) + "kB";
        }
        ctx.fillStyle = "black";
        ctx.fillText(legend, x_start, yoffset + 12);
        x_start += ctx.measureText(legend).width + 5;
    ...
    if (event.bw) {
        if (event.bw > 1e3) {
            legend = (event.bw / 1e3).toFixed(1) + "Mbps";
        } else {
...
function showMetrics() {
    ...
    $("#bufferWindow_c")[0].width = $("#bitrateTimerange_c")[0].width = $("#bufferTimerange_c")[0].width = $("#videoEvent_c")[0].width = $("#metricsButton")[0].width = $("#loadEvent_c")[0].width = width;
    $("#bufferWindow_c").show();
